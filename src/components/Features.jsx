...
const BentoTilt = ({ children, className = "" }) => {
  ...
  const handleMouseMove = (event) => {
    ...
    const relativeY = (event.clientY - left) / width;
    ...
    <div className="relative size-full">
      ...
      <div className="relative z-10 flex size-full flex-col justify-between p-5 text-blue-50">
        <div>
          ...
          {description && (
            <p className="mt-3 max-w-64 text-xs md:text-base bg-black/40 p-2 rounded-xl text-blue-50">{description}</p>
          )}
        </div>
      </div>
    </div>
      ...
      <div className='container mx-auto px-3 md:px-10'>
        ...
        <div className="grid h-[135vh] grid-cols-2 grid-rows-3 gap-7">
          <BentoTilt className="bento-tilt_1 row-span-1 md:col-span-1 md:row-span-2">
            <BentoCard src="/videos/feature-2.mp4" title={<>zig<b>m</b>a</>} description="An anime game and gaming inspired NFT collection - the IP primed for expansion." />
          </BentoTilt>
